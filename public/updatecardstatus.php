...
if (isset($_GET["deck"]) && isset($_GET["card"]) && isset($_GET["date"]) && isset($_GET["status"]) && isset($_GET["difficulty"])) {
    ...
    $date = $_GET["date"];
    $_GET["status"] == "learn" ? $status = true : $status = false;
    ...
    $users->updateOne(array("_id" => $_SESSION["userId"]), array('$set' => array("decks.$deck_id.cards.$card_id.learned" => $status, "decks.$deck_id.cards.$card_id.review_date" => $date, "decks.$deck_id.cards.$card_id.difficulty_level" => $level)));
    foreach ($_SESSION["decks"][$deck_id]["cards"] as $elementKey => $element) {
        foreach ($element as $valueKey => $value) {
            if ($valueKey == 'english' && $value === $card_name) {
                if ($status) {
                    $_SESSION["decks"][$deck_id]["cards"][$elementKey]["learned"] = true;
                    $_SESSION["decks"][$deck_id]["cards"][$elementKey]["review_date"] = $date;
                } else {
                    $_SESSION["decks"][$deck_id]["cards"][$elementKey]["learned"] = false;
                    $_SESSION["decks"][$deck_id]["cards"][$elementKey]["review_date"] = "";
                }
